...
PTR-ToF-MS : Proton Transfert Reaction Time of Flight Mass Spectrometry. Acronyme utilisé pour l'instrument de mesure (l\textbf{e} PTR-ToF-M\textbf{Spectrometer}) et pour la méthode d'analyse (l\textbf{a} PTR-ToF-M\textbf{Spectrometry}). Par abus de langage, l'acronyme PTR-MS est régulièrement utilisé à l'oral.
...
Par ailleurs, je souhaite développer un module permettant de convertir les data brutes dans un format adapté à la future plateforme Recherche Data Gouv. Ce module facilitera la production de \emph{data papers} dont j'ai découvert l'existence au cours des \href{https://casuhal2021.sciencesconf.org/}{journées casuHAL 2021}. Grâce à deux demi-journées de formation sur Rmarkdown et sur la création de package, j'ai pu intégrer la génération de rapports automatisés dans le package\footnote{\url{https://daranzolin.github.io/2021-03-03-automated-rmarkdown/}} qui est un premier pas vers l'écriture automatisée des data papers.\footnote{voir également \textcite{boettiger_2015}}

J'ai également découvert et testé les packages et templates permettant de créer sous R un document découpé en chapitre. J'utilisais déjà Rmarkdown depuis un certains temps mais le passage à \href{https://bookdown.org/}{Bookdown} et \href{https://ericmarcon.github.io/memoiR/}{memoiR} a demandé un temps d'adaptation plus long que prévu. Ceci étant, ça n'a pas été en vain puisque j'ai eu plusieurs retours enthousiastes de collègues en vue d'une formation sur ces packages. J'envisage, une fois ce rapport terminé, de développer un clone de memoiR (avec l'accord de l'auteur) sur le style de \href{https://ecofog.github.io/EcoFoG/}{EcoFoG} afin de mettre à disposition aux membres du CEFE un template clé en main. Ceci permettra de favoriser l'écriture de rapports par des logiciels libres et d'améliorer la reproductibilité de la science.
...
L'expérience sur les lavandes présentée ici s'inscrit dans un cadre plus vaste s'intéressant aux phénomènes de pollution à l'Ozone O\textsubscript{3} liés au changement climatique global. Je ne reviendrai pas dans ce rapport sur la partie ozonée. Cependant, nous nous sommes intéressés aux variations journalières dans des conditions non ozonées (plantes témoins).
...
\StringTok{"AUC\_each.group"} \OtherTok{=} \ConstantTok{FALSE}\NormalTok{,  }\CommentTok{\# Si l\textquotesingle{}argument est le nom d\textquotesingle{}un meta colonne}
...
\StringTok{"cal\_plot\_exp"} \OtherTok{=} \ConstantTok{TRUE}\NormalTok{,    }\CommentTok{\# Les axes x et y sont exponentiels ?}
...
Finalement, nous avons fait le choix de garder la fonction \emph{alignSpectra()} de \textbf{MALDIquant}. De plus, ce package nous permet d'avoir une liste de tous les pics détectés. Ainsi, nous avons pour un même jeu de données, trois matrices à utiliser pour les analyses chimiométriques :
...
\caption{Bilinear model obtained from MCR for an HPLC-DAD data set. Expressed as (a) sum of pure signal contributions; (b) sum of the dyads of pure concentration profile and spectra; and (c) product of matrices of pure concentration profiles and spectra. (Image reproduite de \textcite{dejuan_2014} avec l'autorisation de l'éditeur.)}\label{schMCR}
}
\end{figure}

La MCR est une famille de méthodes qui cherche à décomposer l'ensemble des spectres en plusieurs sous-ensembles de produits purs. La figure \ref{schMCR} issue de l'article de \textcite{dejuan_2014} permet de bien comprendre l'objectif d'une MCR. La matrice D, obtenue par l'expérience, est l'expression de deux produits purs S\textsubscript{A}\textsuperscript{T} et S\textsubscript{B}\textsuperscript{T} (deux bouquets de COV pour notre PTR-ToF-MS) de concentration C\textsubscript{A} et C\textsubscript{B}, qui sont les quatre vecteurs que nous cherchons à connaître\footnote{et dont nous ne connaissons rien.}. Les étapes de l'équation sont décrites dans l'article précédemment cité.

De plus, la MCR permet l'ajout de contraintes telles que la ``non-négativité'' qui permet de ne pas avoir de composantes négatives dans les S\textsubscript{i}\textsuperscript{T} (fréquent en ACP) ou la contrainte ``d'égalité'' qui permet par exemple de renseigner un spectre pur de référence.
...
\tightlist
\item
  l'ambigüité de \textbf{permutation}. Il n'y a pas d'ordre dans les spectres purs et leur concentration associée. Le spectre S\textsubscript{i}\textsuperscript{T} ne représente pas plus ou moins l'ensemble du jeu de donnée que le spectre S\textsubscript{j}\textsuperscript{T}.\\
\item
...
\item[Keywords]
Chimiometry, Open Science, PTR-ToF-MS, Spectrometry, Environment.
~\\
...
\item[Mots-Clés]
Chimiométrie, PTR-ToF-MS, Science ouverte, Spectrométrie, Environnement.
~\\
