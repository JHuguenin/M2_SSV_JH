...
COV (ou VOC) : Composé Organique Volatil. Molécule produite par des systèmes biologique et analysée par le PTR-ToF-MS.
...
Enfin, le chapitre \ref{chap3} détaille les opérations mathématiques utilisées pour l'analyse des données. Je développerais les choix faits quand aux prétraitements et à l'alignement des spectres. Je présenterais également les algorithmes implémentés dans le package, en particulier celui de la MCR-ALS.
...
Par ailleurs, je souhaite développer un module permettant de convertir les data brutes dans un format adapté à la futur plateforme Recherche Data Gouv. Ce module facilitera la production de \emph{data paper} dont j'ai découvert l'existence au cours des \href{https://casuhal2021.sciencesconf.org/}{journées casuHAL 2021}. Grâce à deux demi-journées de formation sur Rmardown et sur la création de package, j'ai pu intégrer la génération de rapport automatisé dans le package\footnote{\url{https://daranzolin.github.io/2021-03-03-automated-rmarkdown/}} qui est un premier pas vers l'écriture automatisée des data paper.\footnote{voir également \textcite{boettiger_2015}}
